...
void game::end ()
{
	...
	{
		ifstream input_file("text.txt",ios::in);
		if (!input_file)
		{
...
void game::play()
{
	...
	for(int j=0;j<3&&x2!=780;j++)
	{
		if(player=='y'||player=='Y')
		{
    		...
    		input_file >>name>>i>>s>>k>>s1>>s2>>s3>>u>>d>>b1>>b2>>a1>>a2>>m>>j;
		}
		char score[3]={k};
		char score1[3]={s};
		char score2[3]={i};
		...
		for(;x2!=780;)
			...
			fstream output_file("text.txt",ios::out);
			output_file <<name<<" "<<i<<" "<<s<<" "<<" "<<k<<" "<<s1<<" "<<s2<<" "<<s3<<" "<<u<<" "<<d<<" "<<b1<<" "<<b2<<" "<<a1<<" "<<a2<<" "<<m<<" "<<j+1;
			if(!output_file)
 		    {
...
int main()
{
	...
	if(n==2)
	{
		cout << "If you continue game press 'y' else press any key to start the game.";
		cin >> a.player;
		if(a.player!='y'&&a.player!='Y')
		a.getname();
		initwindow(800,600);
		a.start();
		a.play();	
